...
import { client, whatsappListo } from './client.js';
import { ejecutarRemarketing, programarRemarketing } from './remarketing.js';
...
/* Servimos el dashboard.html como archivo estático */
...
const DELAY_ENTRE_MENSAJES = 8000; /* 8 segundos entre cada mensaje */
...
app.get('/api/clientes-vencidos', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase
            .from('clientes')
            .select('id,nombre,telefono,ultima_compra')
...
/* Arrancamos también el remarketing automático cuando WhatsApp esté listo */
client.on('ready', () => {
    programarRemarketing(client);
});